...
public class SwitchButton extends View implements View.OnClickListener {
    ...
    public SwitchButton(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.commonCoreSwitchView, defStyleAttr, R.style.common_core_def_switch_view);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.commonCoreSwitchView_switch_bg_color_close) {
                switchViewCloseBgColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.commonCoreSwitchView_switch_bg_color_open) {
                switchViewOpenBgColor = typedArray.getColor(attr, Color.BLACK);

            } else if (attr == R.styleable.commonCoreSwitchView_switch_ball_color) {
